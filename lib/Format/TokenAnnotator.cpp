...
void TokenAnnotator::calculateFormattingInformation(AnnotatedLine &Line) {
  ...
  if( Line._line.ToString()=="std :: thread ( [ pThreadIndex , pContract , excluded , i ] ( ) { } ) . detach ( ) ; " )//"int result = 5 != 7 ; "
     Line._line.ToString();
  if( Line._line.ToString()=="if ( pContract -> Flags != 0 ) " )//"int result = 5 != 7 ; "
     Line._line.ToString();
  while (Current) {
...
{
   if( Left.TokenText==")" && Right.TokenText=="noexcept" )
      return false;
   //if( Left.TokenText=="!" || Right.TokenText=="!" || Left.TokenText=="pContract->Flags" || Right.TokenText=="==" )
   if( Line._line.ToString()=="int x = 0 ; " )
      Line._line.ToString();
   if( Line._line.ToString()=="if ( pContract -> Flags != 0 )" )
      Line._line.ToString();
     ...
     else
     {
        ...
        if( spaces )
        {
           ...
           else
           {
              ...
              for( auto pTok = Right.Next; pTok && spaces; pTok = pTok->Next )
              {
                 if( pTok->is(tok::l_paren) )
                    ++additional;
                 else if( pTok->is(tok::r_paren) )
                 {
                    if( additional==0 )
                        spaces = false;
                    else
  ...
  if (Right.Type == TT_TrailingAnnotation &&
    ...
    // void Fn() const &;
    return Style.PointerAlignment != FormatStyle::PAS_Left;
  return (Right.is(TT_BinaryOperator) && Right.TokenText=="=") 
      || (Left.is(TT_BinaryOperator) && Left.TokenText=="=")
      || (!Right.is(TT_BinaryOperator) && !Left.is(TT_BinaryOperator) );
}
...
bool TokenAnnotator::spaceRequiredBefore(const AnnotatedLine &Line,
  ...
  if( /*(Right.is(TT_BinaryOperator) && !Left.is(tok::l_paren)) ||*/
      //(Left.isOneOf(TT_BinaryOperator, TT_ConditionalExpr) && !Right.is(tok::r_paren)))
     Left.is(TT_ConditionalExpr) && !Right.is(tok::r_paren) )
    return true;
